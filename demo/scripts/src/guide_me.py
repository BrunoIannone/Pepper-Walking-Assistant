...
from map.graph import Node, Graph
...
class SteadyState(TimeoutState):
    ...
    def on_enter(self):
        ...
        global global_mws, hand_picked, alevel
        # if alevel == 0:
        if True:
        ...
        # elif alevel == 1:
            ...
            if hand_picked == 'Left':
                # global_mws.run_interaction(leftBlindWalkHandle)
                global_mws.run_interaction(leftDeafWalkHandle)
            else:
                # global_mws.run_interaction(rightBlindWalkHandle)
                global_mws.run_interaction(leftDeafWalkHandle)
...
class MovingState(State):
    ...
    def on_enter(self):
        ...
        global global_mws
        global_mws.run_interaction(walking)
...
def guide_me(args, mws):
    ...
    graph = Graph.static_load('demo/static/map/graph.txt')
    ...
    global coords
    room_mapper = RoomMapper.static_load('demo/static/map/coords.txt')
